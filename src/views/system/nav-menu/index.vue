  ...
  <el-dialog v-model="dialogVisible" :title="dialogTitle" width="480px">
    <el-form :model="form" label-width="100px">
      ...
      <el-form-item label="链接地址">
        <el-input v-model="form.path" placeholder="/search 或 https://..." />
      </el-form-item>
...
import { navMenuPage, navMenuCreate, navMenuUpdate, navMenuDelete, type INavMenu } from '@/api/navMenu'

defineOptions({ name: 'SystemNavMenuView' })

const activePosition = ref<'top_nav' | 'home_promo'>('top_nav')
const list = ref<INavMenu[]>([])

const dialogVisible = ref(false)
const dialogTitle = ref('新增菜单')
const saving = ref(false)

const form = reactive<INavMenu>({
  title: '',
  path: '',
  position: 'top_nav',
  sort_order: 0,
...
const load = async () => {
  const { data: res } = await navMenuPage({ position: activePosition.value })
  if (res.code !== 200) return
  list.value = res.data?.list || []
}

const openDialog = (row?: INavMenu) => {
  if (row) {
...
const onSave = async () => {
  ...
  try {
    if (form.id) {
      const { data: res } = await navMenuUpdate(form.id, form)
      if (res.code !== 200) return
    } else {
      const { data: res } = await navMenuCreate(form)
      if (res.code !== 200) return
    }
...
const onDelete = async (row: INavMenu) => {
  await ElMessageBox.confirm(`确认删除「${row.title}」吗？`, '提示', { type: 'warning' })
  const { data: res } = await navMenuDelete(row.id!)
  if (res.code !== 200) return
