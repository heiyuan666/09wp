  ...
  <el-card shadow="never">
    ...
    <el-form :inline="true" label-width="80px" class="filter-form">
      ...
      <el-form-item label="游戏ID">
        <el-input v-model="filters.game_id" placeholder="可选" clearable style="width: 160px" />
      </el-form-item>
    ...
    <el-table :data="list" border style="width: 100%; margin-top: 12px" v-loading="loading">
      ...
      <el-table-column prop="game_resource_id" label="资源ID" width="120" />
      <el-table-column label="类型" width="160">
        <template #default="{ row }">
          <el-tag size="small">{{ feedbackTypeLabel(row.type) }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="download_url" label="链接" min-width="260" show-overflow-tooltip />
      <el-table-column prop="extract_code" label="提取码" width="120" />
      ...
      <el-table-column label="操作" width="220" fixed="right">
        <template #default="{ row }">
          <div class="action-cell">
            <el-button link type="primary" @click="openGame(row.game_id)">查看游戏</el-button>
            <el-button
...
import { adminGameFeedbackList, adminGameFeedbackUpdateStatus, type IGameResourceFeedback } from '@/api/gameFeedback'

defineOptions({ name: 'GameFeedbacksView' })

const list = ref<IGameResourceFeedback[]>([])
...
const load = async () => {
  loading.value = true
  try {
    const { data: res } = await adminGameFeedbackList({
      page: page.value,
...
const updateStatus = async (id: number, status: 'pending' | 'processed') => {
  updatingId.value = id
  try {
    const { data: res } = await adminGameFeedbackUpdateStatus(id, status)
    if (res.code !== 200) return
