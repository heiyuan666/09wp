  ...
  <div class="breadcrumb-container" v-if="breadcrumbList.length > 0">
    <el-breadcrumb separator=">" :separator-icon="ArrowRight">
      ...
      <el-breadcrumb-item
        ...
        :key="item.id"
        :to="index < breadcrumbList.length - 1 ? { path: toMenuPath(item.path) } : undefined"
      >
...
const breadcrumbList = computed<BreadcrumbItem[]>(() => {
  ...
  if (!currentMenu) {
    ...
    // 如果找到首页，先添加首页
    if (homeMenu && stripAdminPrefix(route.path) !== homeMenu.path) {
      path.push({
        id: homeMenu.id,
        title: homeMenu.title,
        path: toMenuPath(homeMenu.path || '/'),
        icon: homeMenu.icon,
      })
    }
