/** 与主站 searchHelpers / 游戏详情一致的网盘展示名与排序 */
...
/** 数值越小越靠前（与主站 netdiskLinkDisplayRank 一致） */
export function netdiskLinkSortKey(link: string): number {
...
export function sortNetdiskLinks(urls: string[] | undefined): string[] {
  ...
  return [...urls].sort((a, b) => {
    const d = netdiskLinkSortKey(a) - netdiskLinkSortKey(b)
    if (d !== 0) return d
