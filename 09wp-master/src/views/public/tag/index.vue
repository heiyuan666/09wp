<template>
  <el-card class="panel" shadow="hover">
    <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
      <div>
        <div class="panel-title">标签：{{ tagDisplay }}</div>
    ...
    <div class="resource-grid">
      <div
        v-for="row in shownList"
        :key="row.id"
        class="item-card"
        :class="{ 'item-card--no-cover': hideCovers }"
        @click="goDetail(row.id)"
      >
        <template v-if="!hideCovers">
    ...
    <el-empty v-if="!tagDisplay" description="无效的标签" :image-size="80" />
    <el-empty v-else-if="list.length === 0" description="暂无相关资源" :image-size="80" />

    <div v-if="tagDisplay" class="pager">
      <el-pagination
...
import { siteSearch } from '@/api/netdisk'
...
/** 剧情 标签页不展示封面图（合规/视觉要求） */
const hideCovers = computed(() => tagDisplay.value.trim() === '剧情')
...
const load = async () => {
  const q = tagDisplay.value.trim()
  if (!q) {
    list.value = []
    total.value = 0
    return
  }
  const { data: res } = await siteSearch({
    q,
    page: page.value,
...
watch(
  () => ({ tag: tagDisplay.value, page: page.value, size: pageSize.value }),
  async (cur, prev) => {
    if (prev && cur.tag !== prev.tag) {
      if (cur.page !== 1) {
        page.value = 1
        return
      }
